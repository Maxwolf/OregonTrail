...
namespace TrailSimulation.Core
{
    ...
    /// </summary>
    public sealed class SceneGraphModule : SimulationModule
    {
        /// <summary>
        ...
        ///     renderer is using.
        /// </param>
        private string RenderMode(WindowManagerModule windowManager)
        {
            // If TUI for active game Windows is not null or empty then use it.
            var activeModeTUI = windowManager.ActiveMode?.OnRenderMode();
            if (!string.IsNullOrEmpty(activeModeTUI))
                return activeModeTUI;

            // Otherwise, display default message if null for Windows.
            return windowManager.ActiveMode == null ? GAMEMODE_EMPTY_TUI : GAMEMODE_DEFAULT_TUI;
        }
